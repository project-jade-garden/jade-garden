...
const props = defineProps<{
  ...
  emits?: { "onUpdate:modelValue"?: (data: number) => void };
}>();
const steps = [
  {
    step: 1,
...
<template>
  <StepperRoot data-testid="stepper" v-bind="props.stepperProps">
    <StepperItem
      v-for="item in steps"
      :key="item.step"
      :step="item.step"
    ...
    >
      <StepperTrigger :data-testid="`stepper-item-trigger-${item.step}`">
        ...
        <StepperDescription
          :data-testid="`stepper-item-description-${item.step}`"
        >
          {{ item.description }}
        </StepperDescription>
      </StepperTrigger>
