...
export type CVATraits<T extends Record<string, any>> = {
  [K in keyof T]?: T[K] extends ""
    ? JadeGarden.ClassNameValue | Partial<Record<PropertyKey, JadeGarden.ClassNameValue>>
    : T[K] extends "number"
      ? Partial<Record<PropertyKey, JadeGarden.ClassNameValue>>
      : T[K] extends string
        ? string extends T[K]
          ? never
          : Partial<Record<T[K], JadeGarden.ClassNameValue>>
        : never;
};
 ...
 * ```
 */
type SlotsClassValue<RCV extends JadeGarden.RecordClassValue> = {
 ...
 * ```
 */
type DefaultVariants<RCV extends JadeGarden.RecordClassValue> = {
  [key: string]: {
 ...
 * ```
 */
type CompoundBase<RCV extends JadeGarden.RecordClassValue, V extends JadeGarden.Variants<RCV>> = {
  [K in keyof V]?: JadeGarden.StringToBoolean<keyof V[K]> | JadeGarden.StringToBoolean<keyof V[K]>[];
};
      ...
      : T[K][P] extends "number"
        ...
        : T[K][P] extends string
          ? string extends T[K][P]
            ? never
            : Partial<Record<T[K][P], JadeGarden.ClassNameValue>>
          : never;
  };
};
