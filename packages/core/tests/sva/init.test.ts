...
import { type ClassStrings, cm, sva } from "../../src";

function parseClasses(result: ClassStrings) {
...
declare module "bun:test" {
  interface Matchers<T> {
    toHaveClass(expected: ClassStrings): T;
  }
}
...
describe("sva", () => {
  describe("default", () => {
    ...
    test("with custom class & className", () => {
      ...
      expect(cm(result1, { exclude: "text-3xl" })).toHaveClass(expectedResult);
      expect(cm(result2, { exclude: "text-3xl" })).toHaveClass(expectedResult);
    });
  ...
  describe("slots", () => {
    ...
    describe("with slots", () => {
      ...
      test("default variants, custom class, & className", () => {
        ...
        expect(cm(root({ class: "text-lg" }), { exclude: "text-sm" })).toHaveClass([
          "font-bold",
          "underline",
          "bg-blue-500",
          "text-lg"
        ]);
        expect(cm(root({ className: "text-lg" }), { exclude: "text-sm" })).toHaveClass([
          "font-bold",
          "underline",
          "bg-blue-500",
          "text-lg"
        ...
        // title - duplicates
        expect(cm(title({ class: "text-2xl" }), { include: "text-2xl", exclude: "text-2xl" })).toHaveClass([
          "text-2xl"
        ]);
        expect(cm(title({ className: "text-2xl" }), { include: "text-2xl", exclude: "text-2xl" })).toHaveClass([
          "text-2xl"
        ]);
        // item
        expect(cm(item({ class: "text-sm" }), { exclude: "text-xl" })).toHaveClass(["text-sm", "opacity-100"]);
        expect(list({ className: "bg-blue-50" })).toHaveClass(["list-none", "bg-blue-50"]);
        // list
        expect(cm(wrapper({ class: "flex-row" }), { exclude: "flex-col" })).toHaveClass(["flex", "flex-row"]);
        expect(cm(wrapper({ className: "flex-row" }), { exclude: "flex-col" })).toHaveClass(["flex", "flex-row"]);
      });
      ...
      test("custom variants, custom class, & className", () => {
        ...
        // base
        expect(cm(root({ class: "text-xl" }), { exclude: ["text-3xl", "text-md"] })).toHaveClass([
          "text-xl",
          "font-bold",
          "underline"
        ]);
        expect(cm(root({ className: "text-xl" }), { exclude: ["text-3xl", "text-md"] })).toHaveClass([
          "text-xl",
          "font-bold",
          "underline"
        ]);
        // title
        expect(cm(title({ class: "text-2xl" }), { include: "text-2xl", exclude: ["text-2xl", "text-md"] })).toHaveClass(
          ["text-2xl", "text-white"]
        );
        expect(
          cm(title({ className: "text-2xl" }), { include: "text-2xl", exclude: ["text-2xl", "text-md"] })
        ).toHaveClass(["text-2xl", "text-white"]);
        //item
        expect(cm(item({ class: "bg-purple-50" }), { exclude: "bg-purple-100" })).toHaveClass([
          "text-xl",
          "bg-purple-50",
          "opacity-100"
        ]);
        expect(cm(item({ className: "bg-purple-50" }), { exclude: "bg-purple-100" })).toHaveClass([
          "text-xl",
          "bg-purple-50",
          "opacity-100"
        ]);
        // list
        expect(cm(list({ class: "bg-purple-100" }), { exclude: "bg-purple-200" })).toHaveClass([
          "list-none",
          "bg-purple-100"
        ]);
        expect(cm(list({ className: "bg-purple-100" }), { exclude: "bg-purple-200" })).toHaveClass([
          "list-none",
          "bg-purple-100"
        ]);
        // wrapper
        expect(
          cm(wrapper({ class: "bg-purple-900 flex-row" }), { exclude: ["bg-transparent", "flex-col"] })
        ).toHaveClass(["flex", "bg-purple-900", "flex-row"]);
        expect(
          cm(wrapper({ className: "bg-purple-900 flex-row" }), { exclude: ["bg-transparent", "flex-col"] })
        ).toHaveClass(["flex", "bg-purple-900", "flex-row"]);
      });
    ...
    test("should override the default classes when the variant matches - compoundSlots", () => {
      ...
      expect(cm(tab(), { exclude: "rounded-sm" })).toHaveClass([
        "z-0",
        "w-full",
        "px-3",
        "py-1",
      ...
      expect(cm(tabList(), { exclude: "rounded-md" })).toHaveClass(["flex", "rounded-none"]);
      expect(cm(cursor(), { exclude: "rounded-sm" })).toHaveClass(["absolute", "z-0", "bg-white", "rounded-none"]);
    });
