...
import { builds, targetDir as entry, getPath, outputDir, rootDir } from "./utils";

type TestBuilds = (...args: string[]) => void | Promise<unknown>;
...
describe("plugins", () => {
  ...
  const testBuilds: TestBuilds = (buildName) => {
    beforeEach(() => {
      if (existsSync(targetDir)) rmdirSync(targetDir, { recursive: true });
      mock.clearAllMocks();
    });

    const build = builds[buildName as keyof typeof builds];
  ...
  describe.each(Object.keys(builds))("%s", testBuilds);
});
